...
system "$TEST_COMPILER_HOME/gcc-r217167-install/bin/g++ -o /dev/null -fstack-protector -Wformat -fvisibility-inlines-hidden -Wall -W -O3 -mtune=native  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
system "$TEST_COMPILER_HOME/llvm-r234783-install/bin/clang++ -o /dev/null -O2 --param=ssp-buffer-size=4 -Wformat -O2 -fvisibility=hidden -fvisibility-inlines-hidden  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r207148-install/bin/clang++ -g -O2 small.cpp ";
