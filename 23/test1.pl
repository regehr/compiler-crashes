...
system "$TEST_COMPILER_HOME/gcc-r228461-install/bin/g++ -o /dev/null -O2 -Wformat -Werror=format-security -W -Wall -pedantic -Wpointer-arith -Wwrite-strings -Wno-long-long --std=c++0x -O3  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r120788-install/bin/clang++ --std=c++0x small.cpp ";
