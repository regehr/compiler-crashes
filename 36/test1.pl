...
system "$TEST_COMPILER_HOME/gcc-r221995-install/bin/gcc -o /dev/null -pthread -pthread -g -O2 -O3  -c small.c >/dev/null 2>&1";
exit (-1) unless ($? == 0);
system "$TEST_COMPILER_HOME/llvm-r114139-install/bin/clang -o /dev/null -pthread -O2 -fstack-protector -Werror=format-security -O3 -mtune=native  -c small.c >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r207148-install/bin/clang -g -O2 small.c ";
