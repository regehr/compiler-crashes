...
print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
system "$TEST_COMPILER_HOME/llvm-r242575-install/bin/clang++ -o /dev/null -ftemplate-depth-128 -fstack-protector -Wformat -O3 -pthread -fno-strict-aliasing -O3 -mtune=native  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
  print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
  exit(-1);
}
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r97740-install/bin/clang++ small.cpp ";
