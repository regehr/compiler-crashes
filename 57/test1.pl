...
system "$TEST_COMPILER_HOME/gcc-r205072-install/bin/g++ -o /dev/null -fstack-protector --param=ssp-buffer-size=4 -fno-exceptions --std=c++0x -O3  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r126697-install/bin/g++ small.cpp ";
