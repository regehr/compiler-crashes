...
system "$TEST_COMPILER_HOME/gcc-r202774-install/bin/g++ -o /dev/null -ftemplate-depth-128 -O2 --param=ssp-buffer-size=4 -Wformat -Werror=format-security -Wno-inline -pthread -mtune=native  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
system "$TEST_COMPILER_HOME/llvm-r185338-install/bin/clang++ -o /dev/null -g -fstack-protector -Werror=format-security -finline-functions -Wno-inline -Wall -g -mtune=native  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
