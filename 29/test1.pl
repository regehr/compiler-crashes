...
exit (-1) unless ($? == 0);
system "$TEST_COMPILER_HOME/llvm-r240517-install/bin/clang++ -o /dev/null -ftemplate-depth-128 -Wformat -finline-functions -Wno-inline -Wall -g  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r111994-install/bin/clang++ -O1 small.cpp ";
