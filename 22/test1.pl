...
system "$TEST_COMPILER_HOME/gcc-r146315-install/bin/g++ -o /dev/null -O2 -fstack-protector -O2 -fvisibility=hidden -std=c++0x -fno-exceptions -Wall -O3  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
