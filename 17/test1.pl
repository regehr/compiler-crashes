...
system "$TEST_COMPILER_HOME/gcc-r176391-install/bin/g++ -o /dev/null -O2 -fstack-protector --param=ssp-buffer-size=4 -Wformat -Werror=format-security -O2 -fvisibility=hidden -fvisibility-inlines-hidden -std=c++0x -Wall -W -fPIC  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r157905-install/bin/g++ small.cpp ";
