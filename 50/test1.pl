...
system "$TEST_COMPILER_HOME/gcc-r202373-install/bin/g++ -o /dev/null -ftemplate-depth-128 -g -O2 -fstack-protector -Werror=format-security -O3 -Wall -g -pthread -fvisibility=hidden -Winvalid-pch  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r165771-install/bin/g++ small.cpp ";
