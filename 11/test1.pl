...
system "$TEST_COMPILER_HOME/gcc-r224091-install/bin/g++ -o /dev/null -ftemplate-depth-128 -O2 --param=ssp-buffer-size=4 -Wformat -Wall -g  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
system "$TEST_COMPILER_HOME/llvm-r114139-install/bin/clang++ -o /dev/null -g -O2 -fstack-protector --param=ssp-buffer-size=4 -finline-functions -Wall -g -pthread  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
