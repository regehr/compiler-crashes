...
system "$TEST_COMPILER_HOME/gcc-r196612-install/bin/gcc -o /dev/null -mx32 -g -fstack-protector -fPIC  -c small.c >/dev/null 2>&1";
exit (-1) unless ($? == 0);
system "$TEST_COMPILER_HOME/llvm-r223388-install/bin/clang -o /dev/null -g -Wformat --param -fPIC  -c small.c >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r176391-install/bin/gcc -mx32 --param max-inline-insns-single=1200 small.c ";
