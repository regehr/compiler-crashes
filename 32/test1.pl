...
system "$TEST_COMPILER_HOME/llvm-r240517-install/bin/clang++ -o /dev/null -fstack-protector -Wformat -Werror=format-security -O3 -Wall -fvisibility=hidden -Winvalid-pch -O3 -mtune=native  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r215669-install/bin/clang++ -g -O2 small.cpp ";
