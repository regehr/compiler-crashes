...
system "$TEST_COMPILER_HOME/gcc-r225201-install/bin/g++ -o /dev/null -fstack-protector -Wformat -Werror=format-security -O2 -std=c++0x -fno-exceptions -Wall  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
  print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
  exit(-1);
}
print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
system "$TEST_COMPILER_HOME/llvm-r232274-install/bin/clang++ -o /dev/null -g -O2 -Wformat -Werror=format-security -O2 -fvisibility=hidden -std=c++0x -fno-exceptions  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
  print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
  exit(-1);
}
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r133983-install/bin/clang++ -std=c++0x small.cpp ";
