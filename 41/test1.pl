...
system "$TEST_COMPILER_HOME/gcc-r148834-install/bin/g++ -o /dev/null -g -O2 -fstack-protector --param=ssp-buffer-size=4 -Wformat -Werror=format-security -fvisibility=hidden -fvisibility-inlines-hidden -std=c++0x -W -fPIC -O3  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r123025-install/bin/clang++ small.cpp ";
