...
system "$TEST_COMPILER_HOME/gcc-r212426-install/bin/g++ -o /dev/null -ftemplate-depth-128 -g -Wno-inline -g -pthread -Winvalid-pch  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r162439-install/bin/g++ small.cpp ";
