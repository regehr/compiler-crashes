...
system "$TEST_COMPILER_HOME/llvm-r232274-install/bin/clang++ -o /dev/null -O2 -fstack-protector --param=ssp-buffer-size=4 -Wformat -std=c++0x -fno-exceptions -Wall -O3  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r208767-install/bin/clang++ -g -std=c++0x small.cpp ";
