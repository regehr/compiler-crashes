...
system "$TEST_COMPILER_HOME/gcc-r191236-install/bin/g++ -o /dev/null -g --param=ssp-buffer-size=4 -Wno-c++0x-compat -fvisibility=hidden -fvisibility-inlines-hidden -std=c++0x -Wall -Wno-c++0x-compat  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r168141-install/bin/g++ -std=c++0x small.cpp ";
