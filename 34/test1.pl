...
system "$TEST_COMPILER_HOME/gcc-r148834-install/bin/g++ -o /dev/null -g -Wextra -std=c++0x -pthread -O2  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r122396-install/bin/clang++ small.cpp ";
