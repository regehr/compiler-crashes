...
system "$TEST_COMPILER_HOME/gcc-r146816-install/bin/g++ -o /dev/null -O2 --param=ssp-buffer-size=4 -Wformat -O3 -finline-functions -Wno-inline -Wall -O3  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
  print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
  exit(-1);
}
print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
system "$TEST_COMPILER_HOME/llvm-r201961-install/bin/clang++ -o /dev/null -g --param=ssp-buffer-size=4 -Wall -pthread  -c small.cpp >/dev/null 2>&1";
if ($? != 0) {
  print "validity check failed\n" if defined $ENV{"TEST_DEBUG"};
  exit(-1);
}
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/llvm-r100000-install/bin/clang++ small.cpp ";
