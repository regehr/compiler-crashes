...
system "$TEST_COMPILER_HOME/gcc-r191236-install/bin/g++ -o /dev/null -g -std=c++0x -Wall -fPIC  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r154158-install/bin/g++ -std=c++0x small.cpp ";
