...
system "$TEST_COMPILER_HOME/gcc-r115762-install/bin/g++ -o /dev/null -ftemplate-depth-128 -O2 --param=ssp-buffer-size=4 -Wformat -Werror=format-security -g -pthread  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
system "$TEST_COMPILER_HOME/llvm-r138297-install/bin/clang++ -o /dev/null -ftemplate-depth-128 -g -fstack-protector -finline-functions -Wno-inline -Wall -pthread  -c small.cpp >/dev/null 2>&1";
exit (-1) unless ($? == 0);
my $str = <<EOF;
...
system "rm -f out.txt";
my $err;
my $cmd = "$TEST_COMPILER_HOME/gcc-r110601-install/bin/g++ small.cpp ";
